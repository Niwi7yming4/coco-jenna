...
public class RyokatanaShopMenu extends AbstractContainerMenu {

    public RyokatanaShopMenu(int id, Inventory inv) {
        super(ModMenuTypes.RYOKATANA_SHOP.get(), id);
        addPlayerInventory(inv);
        addPlayerHotbar(inv);
    }

    public RyokatanaShopMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv);
    }

    public void buy(Player player, int offerIndex) {
        if (player instanceof net.minecraft.server.level.ServerPlayer sp) {
            RyokatanaShopOffers.tryPurchase(sp, offerIndex);
        }
    }
